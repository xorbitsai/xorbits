...
export default class DAGChart extends React.Component {
  ...
  getNodeStyle(nodeId) {
    if (nodeId !== this.lastSelectedId) {
      return (
        'cursor: pointer; stroke: #333; fill: url(#progress-' + nodeId + ')'
      )
    } else {
      return (
        'cursor: pointer; stroke: #ff5252; stroke-width: 3; fill: url(#progress-' +
        nodeId +
        ')'
      )
    }
  ...
  componentDidUpdate(prevProps, prevStates, snapshot) {
    ...
    if (
      ...
      this.props.nodes.forEach((node) => {
         ...
         */
        let nodeProgressGradient = inner
        ...
        if (nodeDetail === undefined) {
          ...
          nodeProgressGradient
            ...
            .attr(
              'stop-color',
              this.state.nodeStatusMap[nodeDetail.status].color
            )
      ...
      this.props.dependencies.forEach((dependency) => {
        if (dependency.linkType && dependency.linkType === 1) {
          this.g.setEdge(dependency.fromNodeId, dependency.toNodeId, {
        ...
        if (this.props.onNodeClick) {
          ...
          if (dagNodeId !== this.lastSelectedId) {
            ...
            if (lastSelectedId !== null) {
              this.g.node(lastSelectedId).style =
                this.getNodeStyle(lastSelectedId)
            }
      ...
      d3Select('#' + this.props.graphName)
        ...
        .attr(
          'transform',
          'translate(' +
            (fullWidth - width * initialScale) / 2 +
            ', ' +
            (fullHeight - height * initialScale) / 2 +
            ')'
        )
      ...
      svg.call(zoom, zoom.transform, d3ZoomIdentity.scale(initialScale))
        ...
        if (
          ...
          if (dagNode !== undefined) {
            if (dagNode.style === 'visibility: hidden') {
              dagNode.shape = this.props.nodeShape
              dagNode.style =
                'cursor: pointer; stroke: #333; fill: url(#progress-' +
                node.id +
                ')'
              dagNode.labelStyle = 'cursor: pointer'
            }
  ...
  render() {
    return <svg id={this.props.graphName} style={this.props.dagStyle} />
  }
}

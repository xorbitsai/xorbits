...
export default class NodeResourceTab extends React.Component {
  ...
  generateBandRows() {
    ...
    const tableContent = (converted, band) => {
      const table_style = (
        <TableRow>
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              {Object.keys(converted).map((band) => (
                <Fragment key={band}>{tableContent(converted, band)}</Fragment>
              ))}
            </TableBody>
          </Table>
        </StyledPaper>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell align="right" style={{ width: '40%' }}>
                  {this.state.detail.disk.reads.toFixed(2)}
                </TableCell>
                <TableCell align="right" style={{ width: '40%' }}>
                  {this.state.detail.disk.writes.toFixed(2)}
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell style={{ width: '20%' }}>Network</TableCell>
                <TableCell align="right" style={{ width: '40%' }}>
                  {this.state.detail.network.receives.toFixed(2)}
                </TableCell>
                <TableCell align="right" style={{ width: '40%' }}>
                  {this.state.detail.network.sends.toFixed(2)}
                </TableCell>
              </TableRow>
                ...
                <Fragment>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">
                      {this.state.detail.iowait.toFixed(2)}
                    </TableCell>
                  </TableRow>
                </Fragment>
              )}
            ...
            <TableBody>
              {Object.keys(this.state.detail.disk.partitions).map((path) => (
                <TableRow key={path}>
                  ...
                  <TableCell align="right">
                    {toReadableSize(
                      this.state.detail.disk.partitions[path].size_used
                    )}
                  </TableCell>
                  <TableCell align="right">
                    {toReadableSize(
                      this.state.detail.disk.partitions[path].size_total
                    )}
                  </TableCell>
                  <TableCell align="right">
                    {toReadableSize(
                      this.state.detail.disk.partitions[path].inode_used
                    )}
                  </TableCell>
                  <TableCell align="right">
                    {toReadableSize(
                      this.state.detail.disk.partitions[path].inode_total
                    )}
                  </TableCell>
                </TableRow>
              ))}
            ...
            <TableBody>
              {Object.keys(this.state.detail.quota).map((band) => (
                <TableRow key={`${band}-quota`}>
                  ...
                  <TableCell align="right">
                    {toReadableSize(this.state.detail.quota[band].hold_size)}
                  </TableCell>
                  <TableCell align="right">
                    {toReadableSize(
                      this.state.detail.quota[band].allocated_size
                    )}
                  </TableCell>
                  <TableCell align="right">
                    {toReadableSize(this.state.detail.quota[band].quota_size)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
            ...
            <TableBody>
              {Object.keys(this.state.detail.slot).map((band) => (
                <TableRow key={`${band}-slot`}>
                  <TableCell>{band}</TableCell>
                  <TableCell align="right">
                    {this.state.detail.slot[band].length}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
              ...
              {Object.keys(this.state.detail.storage).map((band) => (
                <Fragment key={band}>
                  <TableRow>
                    <TableCell
                      rowSpan={
                        Object.keys(this.state.detail.storage[band]).length + 1
                      }
                    >
                      {band}
                    </TableCell>
                  ...
                  {Object.keys(this.state.detail.storage[band]).map((level) => (
                    <TableRow key={level}>
                      ...
                      <TableCell align="right">
                        {toReadableSize(
                          this.state.detail.storage[band][level].size_used
                        )}
                      </TableCell>
                      <TableCell align="right">
                        {toReadableSize(
                          this.state.detail.storage[band][level].size_total
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
  ...
  render() {
    ...
    return (
      <Grid container spacing={2}>
        ...
        {Boolean(this.state.detail.disk.partitions) && this.generateDiskRows()}

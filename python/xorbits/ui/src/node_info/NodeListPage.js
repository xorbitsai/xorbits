...
class NodeList extends React.Component {
  ...
  refreshInfo() {
    ...
    fetch(
      `api/cluster/nodes?role=${roleId}&resource=1&detail=1&exclude_statuses=-1`
    )
      .then((res) => res.json())
      .then((res) => {
        const { state } = this
  ...
  render() {
    ...
    const getSharedMemoryInfo = (nodeDetail) => {
      ...
      return `${toReadableSize(memoryInfo['size_used'])} / ${toReadableSize(
        memoryInfo['size_total']
      )}`
    }
...
export default function NodeListPage(props) {
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Title>{`${props.nodeRole.replace(/\w/, (first) =>
          first.toUpperCase()
        )}s`}</Title>
      </Grid>
      <Grid item xs={12}>

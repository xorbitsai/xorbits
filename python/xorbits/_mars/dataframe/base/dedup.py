...
class DataFrameDedup(DataFrameOperand, DataFrameOperandMixin):
    ...
    def execute_uf(cls, ctx: Union[dict, Context], op: "DataFrameDedup"):
        ...
        embedded = input_data.apply(func, axis=1)
        embedded["__signatures__"] = embedded["__signatures__"].apply(
            lambda x: list(enumerate(x))
        )
        clusters = (
            embedded.explode("__signatures__")
            .groupby("__signatures__")["__id__"]
            .apply(set)
        )
    ...
    def execute(cls, ctx: Union[dict, Context], op: "DataFrameDedup"):
        if op.stage == OperandStage.map:
            ...
            input_idx = op.pure_depends.index(False)
            input_data = ctx[op.inputs[input_idx].key]
            out = op.outputs[0]
            union_find = ctx.get_remote_object(op.union_find_name)
            uf = union_find.get_self()
            print(input_data[input_data["id"].map(lambda x: uf.find(x) == x)])
    ...
    def _update_key(self):
        ...
        self._obj_set(
            "_key", tokenize(type(self).__name__, *self._values_, self._pure_depends)
        )
        return self
    ...
    def tile(cls, op: "DataFrameDedup"):
        ...
        print([(c.key, c.op.key) for c in chunks])
        ...
        for i, c in enumerate(chunks):
            ...
            new_op = op.copy().reset_key()
            new_op.union_find_name = union_find_name
            pure_depends = [True] * len(chunks)
            pure_depends[i] = False
        ...
        print([(c.key, c.op.key) for c in cal_chunks])

        new_nsplits = (np.nan,) * len(cal_chunks), (cal_chunks[0].shape[1],)
        new_op = op.copy()
        kw = out_df.params.copy()
        kw.update(dict(chunks=cal_chunks, nsplits=new_nsplits))

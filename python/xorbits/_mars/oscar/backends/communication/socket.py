...
from ....constants import MARS_UNIX_SOCKET_DIR, MARS_UNIX_SOCKET_DIR_WIN
...
def _get_or_create_default_unix_socket_dir():
    ...
    if sys.platform.startswith("win"):
        unix_socket_dir = MARS_UNIX_SOCKET_DIR_WIN
    else:
        unix_socket_dir = MARS_UNIX_SOCKET_DIR
...
def _gen_unix_socket_default_path(process_index):
    return f"{_get_or_create_default_unix_socket_dir()}/{md5(to_binary(str(process_index))).hexdigest()}"  # nosec
